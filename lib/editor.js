...
ChunkedString.prototype.findAcquire = function findAcquire(text) {
  if (text.length == 0) throw Error("Texto de pesquisa vazio não permitido");
  var index = this.text.indexOf(text);
  if (index == -1)
    throw Error("A substring não foi encontrada. Pode ser necessário o envolvimento em tildes.");
  if (index != this.text.lastIndexOf(text))
    throw Error("Existem várias instâncias da substring passada");
...
ChunkedString.prototype.acquire = function acquire(offset, length) {
  ...
  for (var i = 0; i < this.chunks.length; i++) {
    ...
    if (offset + length > c.offset || c.offset + c.text.length > offset)
      throw Error("Sobreposições de pedaços");
  }
...
ChunkedString.prototype.release = function release(chunk) {
  if (this.chunks.indexOf(chunk) == -1) throw Error("Pedaço inválido dado");
  this.chunks.splice(index, 1);
};

ChunkedString.prototype.modify = function modify(chunk, text) {
  if (this.chunks.indexOf(chunk) == -1) throw Error("Pedaço inválido dado");
  if (text.length == 0) throw Error("Substituição vazia não permitida");
...
function Editor(reply, file, encoding) {
  ...
  if (contents.length > 1500 || contents.split("\n") > 50)
    throw Error("O arquivo é muito longo");
...
Editor.prototype.attemptSave = function attemptSave() {
  ...
  process.nextTick(function () {
    ...
    try {
      ...
      this.reply.html("Não foi possível salvar o arquivo: %s", e.message);
      return;
    }
