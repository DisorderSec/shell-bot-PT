...
Command.prototype._ptyData = function _ptyData(chunk) {
  ...
  if ((typeof chunk !== "string") && !(chunk instanceof String))
    throw new Error("Esperava um String, seu mentiroso.");
...
Command.prototype._exit = function _exit(code, signal) {
  ...
  if ((Date.now() - this.startTime) < 2000 && !signal && code === 0 && !this.interacted) {
    ...
    if (signal)
      this.reply.html("\uD83D\uDC80 <strong>Morto</strong> por %s.", utils.formatSignal(signal));
    else if (code === 0)
      this.reply.html("\u2705 <strong>Executado</strong> corretamente.");
    else
      this.reply.html("\u26D4 <strong>Executado</strong> com %s.", code);
  }
